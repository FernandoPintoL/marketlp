...
const fetchList = async (page = 1) => {
    datas.isLoad = true;
    try {
        const response = await axios.post('/compra/query', {
            search: query.value,
            page: page,
        ...
        AlertService.error('Error al cargar las compras');
    } finally {
        datas.isLoad = false;
    }
};
...
const destroyItem = (id) => {
    AlertService.confirm('Esta información ya no podrá ser recuperada').then(async (result) => {
        if (result.isConfirmed) {
            datas.isLoad = true;
            try {
                const response = await axios.delete(`/compra/${id}`);
                if (response.data.isSuccess) {
                    await AlertService.success('La compra se eliminó exitosamente!.');
                    await queryList('');
                } else {
                ...
                AlertService.error('Error al eliminar la compra');
            } finally {
                datas.isLoad = false;
            }
        }
        ...
        <div class="flex flex-col gap-4 p-4">
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    <Link v-if="datas.userType === 'cashier'" :href="route('compra.create')" class="btn btn-primary">
                        Nueva Compra (Cajero)
                    </Link>
                    <Link :href="route('compra.create')" class="btn btn-secondary">
                        Nueva Compra (Cliente)
                    </Link>
                </div>
            </div>
            ...
            <TableLayout v-else>
                ...
                <template #body>
                    <tr v-for="item in datas.list" :key="item.id" class="border-b">
                        ...
                        <td class="px-4 py-2">{{ item.proveedor ? item.proveedor.nombre : 'N/A' }}</td>
                        ...
                        <td class="px-4 py-2">
                            <div class="flex gap-2">
                                <Link :href="route('compra.show', item.id)" class="btn btn-sm btn-info">
                                    Ver
                                </Link>
                                <Link :href="route('compra.edit', item.id)" class="btn btn-sm btn-warning">
                                    Editar
                                </Link>
                    ...
                    <tr v-if="datas.list.length === 0">
                        <td colspan="6" class="px-4 py-2 text-center">No hay compras registradas</td>
                    </tr>
                </template>
            </TableLayout>
